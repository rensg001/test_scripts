...
if [ "${CACHE_SERVICE}" != "" ]; then
    echo "setting memcached service ..."
...
if [ "${RABBITMQ_SERVICE}" != "" ]; then
    echo "setting rabbitmq service ..."
    resolve_service ${RABBITMQ_SERVICE}
    export RABBITMQ_HOST=$RESOLVE_IP
    export RABBITMQ_PORT=$RESOLVE_PORT
fi
